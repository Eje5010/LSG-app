...
window.toggleExtra = (type) => {
    document.getElementById(`${type.toLowerCase()}InputArea`).style.display = document.getElementById(`check${type}`).checked ? 'block' : 'none';
};
...
function renderPrayers() {
    ...
    list.innerHTML = sorted.map(p => {
        ...
        return `<div class="feed-card status-${p.status || 'active'}">
            ...
            ${!isArchive ? `<div class="prayer-actions"><button class="prayed-btn" onclick="window.incrementTally('prayers','${p.id}','tally')">${p.status === 'praise' ? 'Amen!' : 'I Prayed!'}</button> ${p.tally ? `<span class="tally-count">🙏 ${p.tally}</span>` : ''}</div>` : ''}
        </div>`;
    }).join('');
}
...
function renderStudy(date) {
    ...
    else { const isW = new Date(date.replace(/-/g, '/')).getDay() === 3; mc.style.display = isW ? 'block' : 'none'; mi.innerHTML = `<p style="color:#e67e22;">No meal sign-up yet.</p>`; }
}
